...
description: Update codex-sdk-go from the latest stable upstream openai/codex Rust release, review compatibility, prove deterministic generation, run the complete local gate, and carry the reviewed change through a protected pull request to GitHub's automated E2E and publication. Use for protocol refreshes or when preparing a new codex-sdk-go release from an upstream rust-vMAJOR.MINOR.PATCH tag.
---
...
5. The merge of `.github/sdk-version` starts `.github/workflows/release.yml`. Find that run and report its URL. Monitor validation, Quality, E2E, and publication through completion. The happy path requires no human approval: keep both `e2e` and `release` restricted to protected branches with no required reviewers. If an environment requests approval, restore this configured policy while preserving other protections; do not approve the pending job on the user's behalf. Fix failed checks within the authorized scope; report any blocker requiring user input.
